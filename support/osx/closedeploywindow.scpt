...
tell application "System Events"
	...
	tell process "TextMate"
		repeat with i from (count windows) to 1 by -1 -- iterate backwards so indices don't shift on close
			try				
				if name of window i is "Deploy To Server" then
					--tell application "TextMate" to set miniaturized of window i to false					
...
if closedCount is 0 then
	return "No HTML output windows to close."
else if closedCount is 1 then
